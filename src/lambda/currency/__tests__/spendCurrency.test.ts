/**
 * Tests for spendCurrency Lambda function
 */

import { handler } from '../spendCurrency';
...
describe('spendCurrency Lambda', () => {
  ...
  it('should successfully spend currency', async () => {
    ...
    expect(responseBody.transaction.type).toBe('spent');
    expect(responseBody.newBalance).toBe(400);
  ...
  it('should reject when insufficient funds', async () => {
    const poorCharacter = {
      ...mockCharacter,
      currency: 50, // Less than requested amount
    };

    mockDatabaseService.getItem.mockResolvedValueOnce(poorCharacter);
    ...
    expect(responseBody.currentBalance).toBe(50);
    expect(responseBody.requiredAmount).toBe(100);
  });
  ...
  it('should handle character with no existing currency', async () => {
    ...
    expect(result.statusCode).toBe(400);
    expect(JSON.parse(result.body).error).toBe('Insufficient funds');
  });

  it('should spend exact balance amount', async () => {
    mockDatabaseService.getItem.mockResolvedValueOnce(mockCharacter);
    mockDatabaseService.updateItem.mockResolvedValueOnce(undefined);
    mockDatabaseService.putItem.mockResolvedValueOnce(undefined);
    ...
    const responseBody = JSON.parse(result.body);
    expect(responseBody.newBalance).toBe(0);
  });
