...
exports.TaskValidationService = void 0;
const taskQueue_1 = require("../types/taskQueue");
class TaskValidationService {
    static validateTask(task, playerStats, playerLevel, playerInventory, options = {}) {
        const errors = [];
        const warnings = [];
    ...
    static validateBasicTask(task, errors) {
    ...
    static shouldBypassValidation(options) {
        return !!(options.bypassValidation || options.adminOverride || options.testMode);
    }
    static logValidationBypass(task, options) {
        const reason = options.bypassReason || taskQueue_1.ValidationBypassReason.TESTING;
        console.log(`[VALIDATION BYPASS] Task ${task.id} validation bypassed. Reason: ${reason}`);
        if (typeof window !== 'undefined' && window.gameAuditLog) {
            window.gameAuditLog.push({
    ...
    static validateEquipmentRequirements(task, playerStats, errors, warnings) {
        switch (task.type) {
            case taskQueue_1.TaskType.HARVESTING:
    ...
    static validateHarvestingEquipment(task, errors, warnings) {
    ...
    static validateCraftingEquipment(task, errors, warnings) {
    ...
    static validateCombatEquipment(task, errors, warnings) {
    ...
    static validateHarvestingTask(task, playerStats, playerLevel, errors, warnings, options = {}) {
        const harvestingData = task.activityData;
    ...
    static validateCraftingTask(task, playerStats, playerLevel, playerInventory, errors, warnings, options = {}) {
        const craftingData = task.activityData;
    ...
    static validateCombatTask(task, playerStats, playerLevel, errors, warnings, options = {}) {
        const combatData = task.activityData;
        if (!combatData.enemy) {
            errors.push({
    ...
    static validatePrerequisites(prerequisites, playerStats, playerLevel, playerInventory, errors) {
    ...
    static validateResourceRequirements(requirements, playerInventory, errors) {
    ...
    static checkPrerequisite(prerequisite, playerStats, playerLevel, playerInventory) {
        switch (prerequisite.type) {
            case 'level':
                return playerLevel >= prerequisite.requirement;
            case 'stat':
                const statValue = this.getPlayerStat(playerStats, prerequisite.requirement);
                return statValue >= prerequisite.value;
            case 'skill':
                const skillValue = this.getSkillLevel(playerStats, prerequisite.requirement);
                return skillValue >= prerequisite.value;
            case 'item':
                const itemCount = playerInventory[prerequisite.requirement] || 0;
                return itemCount >= prerequisite.value;
            case 'equipment':
                return true;
            case 'activity':
                return true;
    ...
    static getSkillLevel(playerStats, skillName) {
    ...
    static validateResourceAvailability(requirements, playerInventory, options = {}) {
        const errors = [];
        const warnings = [];
        if (options.skipResourceCheck) {
    ...
    static validatePrerequisiteRequirements(prerequisites, playerStats, playerLevel, playerInventory, options = {}) {
        const errors = [];
        const warnings = [];
        if (options.skipPrerequisiteCheck) {
            return { isValid: true, errors: [], warnings: [] };
    ...
    static checkPrerequisiteDetailed(prerequisite, playerStats, playerLevel, playerInventory) {
        switch (prerequisite.type) {
            case 'level':
                const requiredLevel = prerequisite.requirement;
            ...
            case 'stat':
                const statName = prerequisite.requirement;
                const requiredValue = prerequisite.value;
            ...
            case 'skill':
                const skillName = prerequisite.requirement;
                const requiredSkill = prerequisite.value;
            ...
            case 'item':
                const itemId = prerequisite.requirement;
                const requiredQuantity = prerequisite.value;
    ...
    static createAdminValidationOptions(reason = 'Admin override') {
        return {
            bypassValidation: true,
            bypassReason: taskQueue_1.ValidationBypassReason.ADMIN_OVERRIDE,
            adminOverride: true,
    ...
    static createTestValidationOptions() {
        return {
            bypassValidation: true,
            bypassReason: taskQueue_1.ValidationBypassReason.TESTING,
            testMode: true,
            allowInvalidTasks: true
        };
    }
    static createDebugValidationOptions() {
        return {
            bypassValidation: false,
            bypassReason: taskQueue_1.ValidationBypassReason.DEBUG,
            testMode: true,
            allowInvalidTasks: true,
    ...
    static validateTaskWithAudit(task, playerStats, playerLevel, playerInventory, options = {}, auditContext) {
    ...
    static validateTaskComprehensive(task, playerStats, playerLevel, playerInventory, options = {}) {
        const errors = [];
        const warnings = [];
        try {
            if (this.shouldBypassValidation(options)) {
    ...
    static validateBasicTaskComprehensive(task, errors, warnings) {
        if (!task.id || task.id.trim() === '') {
            errors.push({
                code: 'INVALID_TASK_ID',
                message: 'Task ID is required and cannot be empty',
        ...
        else if (task.duration > 24 * 60 * 60 * 1000) {
            errors.push({
        ...
        else if (task.duration > 4 * 60 * 60 * 1000) {
            warnings.push({
                code: 'LONG_DURATION',
    ...
    static validateHarvestingTaskComprehensive(task, playerStats, playerLevel, errors, warnings, options = {}) {
        const harvestingData = task.activityData;
    ...
    static validateCraftingTaskComprehensive(task, playerStats, playerLevel, playerInventory, errors, warnings, options = {}) {
        const craftingData = task.activityData;
    ...
    static validateCombatTaskComprehensive(task, playerStats, playerLevel, errors, warnings, options = {}) {
        const combatData = task.activityData;
        ...
        if (combatData.estimatedOutcome) {
            ...
            if (outcome.estimatedDuration > 300) {
                warnings.push({
    ...
    static validatePrerequisitesComprehensive(prerequisites, playerStats, playerLevel, playerInventory, errors, warnings) {
    ...
    static validateResourceRequirementsComprehensive(requirements, playerInventory, errors, warnings) {
    ...
    static validateEquipmentRequirementsComprehensive(task, playerStats, errors, warnings) {
        switch (task.type) {
            case taskQueue_1.TaskType.HARVESTING:
    ...
    static validateHarvestingEquipmentComprehensive(task, errors, warnings) {
        const harvestingData = task.activityData;
        if (!harvestingData.tools || harvestingData.tools.length === 0) {
            warnings.push({
                code: 'NO_HARVESTING_TOOLS',
    ...
    static validateCraftingEquipmentComprehensive(task, errors, warnings) {
        const craftingData = task.activityData;
        if (craftingData.craftingStation) {
            const station = craftingData.craftingStation;
    ...
    static validateCombatEquipmentComprehensive(task, errors, warnings) {
        const combatData = task.activityData;
        if (!combatData.equipment || combatData.equipment.length === 0) {
            warnings.push({
                code: 'NO_COMBAT_EQUIPMENT',
    ...
    static validateTaskComplexity(task, errors, warnings) {
        ...
        if (task.duration < 5000 && complexityScore > 2) {
            warnings.push({
        ...
        if (task.duration > 3600000 && complexityScore === 0) {
            warnings.push({
        ...
        if (task.maxRetries > 0 && task.type === taskQueue_1.TaskType.COMBAT) {
            warnings.push({
        ...
        if (task.priority > 8 && task.duration > 1800000) {
            warnings.push({
    ...
    static performCrossValidation(task, playerStats, playerLevel, playerInventory, errors, warnings) {
        const levelPrereqs = task.prerequisites.filter(p => p.type === 'level');
        const itemPrereqs = task.prerequisites.filter(p => p.type === 'item');
        if (levelPrereqs.length > 0 && task.resourceRequirements.length === 0 && task.type !== taskQueue_1.TaskType.COMBAT) {
            warnings.push({
        ...
        const expectedDurations = {
            [taskQueue_1.TaskType.HARVESTING]: { min: 10000, max: 300000 },
            [taskQueue_1.TaskType.CRAFTING]: { min: 30000, max: 1800000 },
            [taskQueue_1.TaskType.COMBAT]: { min: 5000, max: 600000 }
        };
    ...
    static validateTaskQueue(tasks, maxQueueSize, maxTotalDuration) {
        const errors = [];
        const warnings = [];
    ...
    static validateTaskBatch(tasks, playerStats, playerLevel, playerInventory, options = {}) {
        const results = {};
    ...
    static validateTaskWithContext(task, playerStats, playerLevel, playerInventory, context, options = {}) {
        const contextualOptions = { ...options };
        if (context.gameMode === 'creative') {
            contextualOptions.bypassValidation = true;
            contextualOptions.bypassReason = taskQueue_1.ValidationBypassReason.DEBUG;
        }
    ...
    static createEmergencyBypassOptions(reason) {
        return {
            bypassValidation: true,
            bypassReason: taskQueue_1.ValidationBypassReason.EMERGENCY,
            adminOverride: true,
            skipResourceCheck: true,
            skipPrerequisiteCheck: true,
            skipEquipmentCheck: true,
    ...
    static createMaintenanceBypassOptions() {
        return {
            bypassValidation: true,
            bypassReason: taskQueue_1.ValidationBypassReason.MAINTENANCE,
            adminOverride: true,
            skipResourceCheck: true,
            skipPrerequisiteCheck: true,
            skipEquipmentCheck: true,
    ...
    static validateTaskProgressive(task, playerStats, playerLevel, playerInventory, strictnessLevel = 'normal') {
        const options = {};
        switch (strictnessLevel) {
            case 'lenient':
                options.skipResourceCheck = false;
                options.skipPrerequisiteCheck = false;
                options.skipEquipmentCheck = true;
    ...
    static validateTaskWithPerformanceMonitoring(task, playerStats, playerLevel, playerInventory, options = {}) {
    ...
    static exportValidationConfig() {
        return {
            supportedTaskTypes: [taskQueue_1.TaskType.HARVESTING, taskQueue_1.TaskType.CRAFTING, taskQueue_1.TaskType.COMBAT],
            validationRules: {
            ...
            bypassReasons: Object.values(taskQueue_1.ValidationBypassReason),
            errorCodes: [
...
exports.TaskValidationService = TaskValidationService;
